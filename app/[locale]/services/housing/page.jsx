...
export default function HousingPage() {
  ...
  const heroData = t('housing.hero', { returnObjects: true }) || {};
  const introData = t('housing.intro', { returnObjects: true }) || {};
  const individualsData = t('housing.forIndividuals', { returnObjects: true }) || {};
  const employersData = t('housing.forEmployers', { returnObjects: true }) || {};
  const providersData = t('housing.forProviders', { returnObjects: true }) || {};
  const whyChooseUsData = t('housing.whyChooseUs', { returnObjects: true }) || {};
  const contactData = t('housing.contact', { returnObjects: true }) || {};

  const forIndividualsServices = Array.isArray(individualsData.services) ? individualsData.services : [];
  const forEmployersServices = Array.isArray(employersData.services) ? employersData.services : [];
  const providerSteps = Array.isArray(providersData.process?.steps) ? providersData.process.steps : [];
  const providerBenefits = Array.isArray(providersData.benefits?.items) ? providersData.benefits.items : [];
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FE]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {forIndividualsServices.map((service, idx) => (
            <div key={idx} className="bg-white border border-gray-200 rounded-lg p-6 sm:p-8 lg:p-10">
              ...
              <Typography sx={{ fontSize: { xs: '0.9rem', sm: '0.95rem', lg: '1rem', '4k': '1.0625rem' }, color: '#6B7280', lineHeight: 1.8, mb: 6 }}>
                {service.description}
              </Typography>
      ...
      <div className="max-w-[1400px] 2xl:max-w-[1600px] 4k:max-w-[2400px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 4k:px-24 mb-12 sm:mb-16 lg:mb-20">
        <Typography sx={{ fontSize: { xs: '1.375rem', sm: '1.5rem', md: '1.75rem', lg: '2rem', '4k': '2.5rem' }, fontWeight: 700, mb: 3, color: '#1e293b' }}>
          {employersData.heading}
        </Typography>
        <Typography sx={{ fontSize: { xs: '0.95rem', sm: '1rem', md: '1.0625rem', lg: '1.125rem', '4k': '1.25rem' }, color: '#6B7280', lineHeight: 1.8, mb: 8 }}>
          {employersData.intro}
        </Typography>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 sm:p-8 lg:p-10 4k:p-12 mb-8">
          <ul className="space-y-4">
            {forEmployersServices.map((service, idx) => (
              <li key={idx} className="flex gap-4">
                <span className="flex-shrink-0 text-blue-600 font-bold text-lg">•</span>
                <Typography sx={{ fontSize: { xs: '0.9rem', sm: '0.95rem', lg: '1rem', '4k': '1.0625rem' }, color: '#1e293b', lineHeight: 1.8 }}>
                  {service}
                </Typography>
              </li>
            ))}
      ...
      <div className="max-w-[1400px] 2xl:max-w-[1600px] 4k:max-w-[2400px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 4k:px-24 mb-12 sm:mb-16 lg:mb-20">
        <Typography sx={{ fontSize: { xs: '1.375rem', sm: '1.5rem', md: '1.75rem', lg: '2rem', '4k': '2.5rem' }, fontWeight: 700, mb: 3, color: '#1e293b' }}>
          {providersData.heading}
        </Typography>
        <Typography sx={{ fontSize: { xs: '0.95rem', sm: '1rem', md: '1.0625rem', lg: '1.125rem', '4k': '1.25rem' }, color: '#6B7280', lineHeight: 1.8, mb: 12 }}>
          {providersData.intro}
        </Typography>

        {/* Process Steps */}
        <div className="mb-12">
          <Typography sx={{ fontSize: { xs: '1.25rem', sm: '1.375rem', md: '1.5rem', lg: '1.75rem', '4k': '2rem' }, fontWeight: 700, mb: 8, color: '#1e293b' }}>
            {providersData.process?.heading}
          </Typography>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 sm:p-8 lg:p-10 4k:p-12">
          <Typography sx={{ fontSize: { xs: '1.25rem', sm: '1.375rem', md: '1.5rem', lg: '1.75rem', '4k': '2rem' }, fontWeight: 700, mb: 8, color: '#1e293b' }}>
            {providersData.benefits?.heading}
          </Typography>
          <ul className="space-y-4">
            {providerBenefits.map((benefit, idx) => (
              <li key={idx} className="flex gap-4">
